...
5. 서버가 `AuthIdentity(provider='kakao')` 기준으로 사용자를 찾음
...
- 카카오 이메일은 참고 정보일 뿐, 가입 완료의 필수 기준이 아님

## 2) 카카오 최초 가입 / 온보딩

1. `SIGNUP_REQUIRED` 응답 수신
2. 온보딩 화면에서 닉네임 등 필수 정보 입력
3. 약관 동의
...
1. 로그인한 이메일 회원이 `내 정보` 화면 진입
2. 새 이메일 + 현재 비밀번호 입력
3. `POST /auth/email/change/start`
4. 서버가 `EmailChangeVerification` pending record 생성
5. 개발 단계에서는 `debugCode` 또는 서버 로그로 코드 확인
6. `POST /auth/email/change/verify`
7. 성공 시 `User.email`, `AuthIdentity(email).providerUserId`, `providerEmail` 동시 갱신
8. 내 정보 화면에서 변경 완료 상태 반영

정책:
- 이메일 변경은 이메일 identity가 연결된 계정만 가능
- 현재 비밀번호 확인이 선행되어야 함
- active 동일 이메일 계정으로는 변경 불가
...
7. `LINK_SUCCESS` 응답 후 `linkedProviders` 갱신
...
2. 사용자 문서를 tombstone 상태로 전환
3. 연결된 로그인 수단 제거
4. 친구/알림/읽음 상태 정리
5. direct/group 방 정책에 따라 채팅 문맥 정리
6. 클라이언트 세션 해제 후 로그인 화면 복귀
