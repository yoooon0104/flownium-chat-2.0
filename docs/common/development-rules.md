...
12. 소스 수정이나 기능 추가를 할 때는 해당 흐름의 주석도 같은 작업에서 바로 반영한다. 주석을 나중에 몰아서 보강하지 않는다.
13. 레거시 DB 인덱스/스키마 이슈가 있으면 서버 시작 시점 보정 로직 또는 마이그레이션 계획을 문서화한다.
14. 운영 설정 이슈는 추측보다 로그와 실제 설정값을 먼저 확인한다.
15. 한글이 포함된 소스/문서 파일은 UTF-8로 저장한다. PowerShell 기본 인코딩으로 저장하지 않는다.
16. PowerShell로 파일을 쓸 때는 UTF-8 명시 저장 방식을 사용한다.
