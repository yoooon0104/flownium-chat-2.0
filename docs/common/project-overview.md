...
## 1) Flownium Chat는 무엇인가

Flownium Chat는 친구 기반 1:1/그룹 채팅을 중심으로, 이메일 인증과 카카오 간편로그인을 함께 지원하는 실시간 채팅 서비스입니다.

제품 목표:
- 로그인부터 대화 시작까지 끊기지 않는 흐름 제공
- 친구 관계 기반의 안전한 채팅 경험 제공
- 이후 익명방/비회원 확장까지 고려한 계정 구조 유지
...
- MVP2-B 1차 구현 완료
- 인증/계정 UX 1차 재구성 완료
- 내 정보 화면 계정 상태/피드백 polish 완료
- 브랜딩/아이콘 자산 1차 반영 완료
- 채팅방 목록 unread / direct profile image / 모바일 back-navigation 보정 완료
...
2. 로그인/인증 화면은 시스템 테마를 따른다
3. 브랜드 워드마크는 `on-light / on-dark`를 테마에 맞게 분기한다
...
- 이메일 회원가입과 카카오 로그인은 자동 병합하지 않음
- 이메일 회원은 명시적으로 카카오 계정을 연결할 수 있음
- 회원탈퇴 시 `User`는 tombstone으로 남기고 로그인 수단은 제거
- 같은 카카오 계정 재가입은 기존 tombstone 계정 복구가 아니라 새 사용자 생성 흐름

## 7) 운영 상태

- 로컬/운영 환경 모두 카카오 로그인 1차 검증 완료
- 이메일 회원가입/인증/로그인/비밀번호 재설정은 개발 단계 `debugCode` 기반 검증 가능
- 서버 시작 시 레거시 `roomKey_1`, `users.kakaoId_1` 인덱스를 자동 제거
...
4. 친구 요청/친구 목록 UX 최종 polish
5. 익명방/비회원 확장 scope 상세화
