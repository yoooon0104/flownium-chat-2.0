# 프롬프트 사용 가이드

이 문서는 `.codex/prompts/` 아래에 있는 프롬프트 파일을 언제, 어떻게 쓰면 좋은지 설명하는 가이드입니다.

## 1. 목적

이 프롬프트 파일들은 저장소 공통 규칙과는 별개로, 작업별 실행 모드를 나누기 위한 문서입니다.

목표:
- AI의 작업 범위를 좁히기
- 과한 수정이나 불필요한 행동 줄이기
- 출력 형식과 작업 흐름을 안정적으로 맞추기
- 리뷰, 분석, 버그수정, 기능구현, 워크플로우를 분리하기

## 2. 권한 모델

각 프롬프트에는 `Permission Scope` 섹션이 들어갈 수 있습니다.

기본 원칙:
- `Permission Scope`는 해당 작업의 권한 경계로 본다
- 명시적으로 허용되지 않은 행동은 기본적으로 금지로 본다
- 프롬프트 권한은 현재 작업에 대한 더 좁고 구체적인 제한이다
- 저장소 공통 규칙은 `AGENTS.md`를 따른다

위험 작업 예시:
- 코드 수정
- 문서 수정
- 커밋
- push
- PR 생성
- GitHub Issue 생성

## 3. 어떤 프롬프트를 언제 쓰면 좋은가

### `analyze.md`

이럴 때 사용:
- 요구사항이 아직 모호할 때
- 구현 전에 빠진 질문을 먼저 정리하고 싶을 때
- 범위, 가정, 리스크를 먼저 드러내고 싶을 때

이럴 때는 비추천:
- 이미 바로 코드를 수정해야 할 때

예시 요청:
- `analyze.md로 먼저 분석해줘`

### `review.md`

이럴 때 사용:
- 코드리뷰만 하고 싶을 때
- 버그나 리스크만 뽑고 싶을 때
- 같은 단계에서 수정까지 하길 원하지 않을 때

이럴 때는 비추천:
- 바로 수정까지 시키고 싶을 때

예시 요청:
- `review.md로 이 브랜치 리뷰해줘`

### `bugfix.md`

이럴 때 사용:
- 재현 가능한 버그가 있을 때
- 최소 수정으로 해결하고 싶을 때
- 수정 후 검증까지 보고 싶을 때

이럴 때는 비추천:
- 버그가 아니라 새 기능 구현일 때

예시 요청:
- `bugfix.md로 이 오류 고쳐줘`

### `feature.md`

이럴 때 사용:
- 기능 요청이 이미 어느 정도 명확할 때
- 구현 중심으로 바로 들어가고 싶을 때
- 최소 구현과 검증이 필요할 때

이럴 때는 비추천:
- 요구사항 정리가 먼저 필요한 상황일 때

예시 요청:
- `feature.md 기준으로 이 기능 구현해줘`

### `issue.md`

이럴 때 사용:
- 이슈 후보를 정리하고 싶을 때
- 리뷰 결과를 이슈 초안으로 만들고 싶을 때
- acceptance criteria까지 같이 적고 싶을 때

이럴 때는 비추천:
- 코드 수정이 필요한 작업일 때

예시 요청:
- `issue.md로 이 리뷰 결과 이슈 초안 만들어줘`

### `workflows/issue-to-pr.md`

이럴 때 사용:
- 시작점이 이미 명확한 이슈일 때
- 분석부터 구현, 검증, PR 요약까지 한 흐름으로 처리하고 싶을 때
- end-to-end 작업 흐름이 필요할 때

이럴 때는 비추천:
- 리뷰만 필요한 경우
- 작은 수정만 필요한 경우

예시 요청:
- `issue-to-pr.md로 이 티켓 진행해줘`

## 4. 추천 흐름

### 리뷰 후 수정

순서:
1. `review.md`
2. 어떤 finding을 고칠지 확정
3. `bugfix.md`

이 흐름이 좋은 경우:
- 리뷰와 수정을 분리해서 관리하고 싶을 때

### 분석 후 기능 구현

순서:
1. `analyze.md`
2. 범위와 가정 정리
3. `feature.md`

이 흐름이 좋은 경우:
- 기능 요청이 아직 덜 다듬어진 상태일 때

### 리뷰 후 이슈화

순서:
1. `review.md`
2. 이슈화할 finding 선별
3. `issue.md`

이 흐름이 좋은 경우:
- 리뷰 결과를 백로그성 이슈로 남기고 싶을 때

### 이슈에서 PR까지

순서:
1. 구체적인 이슈 준비
2. `workflows/issue-to-pr.md`
3. 구현, 검증, 요약 진행
4. push/PR은 명시 요청이 있을 때만 수행

이 흐름이 좋은 경우:
- 이미 승인된 작업을 한 번에 진행할 때

## 5. 요청 문장을 어떻게 쓰면 좋은가

좋은 예:
- `review.md로 현재 브랜치 리뷰해줘`
- `bugfix.md로 unread 버그 고쳐줘`
- `analyze.md로 먼저 빠진 조건 정리해줘`
- `issue.md로 이 finding들 이슈 초안 만들어줘`
- `issue-to-pr.md로 이 이슈 진행해줘`

애매한 예:
- `이거 해줘`
- `끝까지 다 처리해줘`

요청이 구체적일수록 결과도 더 안정적입니다.

## 6. 이슈 자동화는 어떻게 생각하면 좋은가

추천 정책:
- `review.md`는 finding을 뽑는다
- `issue.md`는 이슈 초안을 만든다
- 실제 GitHub Issue 생성은 사용자가 명시적으로 요청할 때만 한다

권장 심각도 기준:
- `P0`, `P1`: 기본 이슈 후보
- `P2`: 사용자 승인 시 이슈 후보
- `P3`: 보통은 이슈화하지 않음

이렇게 해야 이슈가 과하게 늘어나지 않고, 실제로 중요한 것만 정리됩니다.

## 7. 유지보수 원칙

프롬프트를 수정할 때는 아래 기준을 지키는 것이 좋습니다.

- `Permission Scope`는 짧고 직접적으로 쓴다
- 출력 형식은 자주 흔들지 않는다
- 역할이 겹치는 프롬프트를 한 파일에 몰아넣지 않는다
- 책임이 충돌하면 기존 프롬프트를 비대하게 키우기보다 workflow 프롬프트를 추가한다
